...
</html>`;return e.html(t)});re.post("/login",async e=>{const t=await e.req.parseBody(),r=e.env.ADMIN_USERNAME||process.env.ADMIN_USERNAME||"kelvin.reallife8@gmail.com",s=e.env.ADMIN_PASSWORD||process.env.ADMIN_PASSWORD||"paraKenya8#";return t.username===r&&t.password===s?(pr(e,"codeward_admin_session",s,{path:"/admin",httpOnly:!0,secure:!0,maxAge:3600*24*7}),e.redirect("/admin")):e.redirect("/admin/login?error=1")});re.get("/logout",e=>(gs(e,"codeward_admin_session",{path:"/admin"}),e.redirect("/admin/login")));const st={"software-engineer":"Software Engineer","senior-engineer":"Senior / Staff Engineer","engineering-lead":"Engineering Lead / Manager","cto-vp":"CTO / VP of Engineering","devops-platform":"DevOps / Platform Engineer","security-engineer":"Security Engineer",freelancer:"Freelance Developer","open-source":"Open Source Contributor",student:"Student / Bootcamp",founder:"Founder / Indie Hacker",other:"Other"};function M(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function we(e){if(e==null)return"";const t=String(e);return/[",\n]/.test(t)?'"'+t.replace(/"/g,'""')+'"':t}re.get("/",async e=>{var Pt,Lt,_t,$t,jt,Dt;const{env:t}=e,r=new URL(e.req.url),s=(r.searchParams.get("q")||"").trim(),n=(r.searchParams.get("role")||"").trim(),a=Math.max(1,parseInt(r.searchParams.get("page")||"1",10)||1),i=25,o=r.searchParams.get("msg")||"";let l="";o==="retrigger_success"?l='<div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); color: #10b981; padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; font-weight: 500; display: flex; align-items: center; gap: 8px;"><svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg> Email successfully resent!</div>':o==="retrigger_failed"&&(l='<div style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: #ef4444; padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; font-weight: 500; display: flex; align-items: center; gap: 8px;"><svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg> Failed to resend email. Please check the logs.</div>');let d="WHERE 1=1";const p=[];let h=1;if(s){d+=` AND (LOWER(name) LIKE $${h++} OR LOWER(email) LIKE $${h++} OR LOWER(company) LIKE $${h++})`;const y=`%${s.toLowerCase()}%`;p.push(y,y,y)}n&&(d+=` AND role = $${h++}`,p.push(n));const f=await Ot(e).connect(),w=await f.query(`SELECT COUNT(*) as cnt FROM waitlist_entries ${d}`,p),v=Number(((Pt=w.rows[0])==null?void 0:Pt.cnt)??0),k=Math.max(1,Math.ceil(v/i)),E=Math.min(a,k),T=(E-1)*i,x=[...p,i,T],I=(await f.query(`SELECT id, name, email, role, company, github, position, created_at, email_sent, linkedin_clicked
     FROM waitlist_entries ${d}
     ORDER BY created_at DESC
     LIMIT $${h++} OFFSET $${h++}`,x)).rows||[],De=await f.query("SELECT COUNT(*) as cnt FROM waitlist_entries"),ve=Number(((Lt=De.rows[0])==null?void 0:Lt.cnt)??0),_=await f.query(`SELECT 
       SUM(CASE WHEN email_sent = 1 THEN 1 ELSE 0 END) as sent,
       SUM(CASE WHEN email_sent = 0 THEN 1 ELSE 0 END) as failed
     FROM waitlist_entries`),pt=Number(((_t=_.rows[0])==null?void 0:_t.sent)??0),Qe=Number((($t=_.rows[0])==null?void 0:$t.failed)??0),se=await f.query("SELECT COUNT(*) as cnt FROM waitlist_entries WHERE created_at::DATE = CURRENT_DATE"),L=Number(((jt=se.rows[0])==null?void 0:jt.cnt)??0),be=(await f.query("SELECT role, COUNT(*) as cnt FROM waitlist_entries GROUP BY role ORDER BY cnt DESC")).rows,Y=await f.query("SELECT COUNT(*) as cnt FROM waitlist_entries WHERE company IS NOT NULL AND company != ''"),ne=Number(((Dt=Y.rows[0])==null?void 0:Dt.cnt)??0),yr=Object.entries(st).map(([y,P])=>`<option value="${y}" ${n===y?"selected":""}>${M(P)}</option>`).join(""),wr=I.length?I.map(y=>{const P=new Date(y.created_at+"Z"),He=isNaN(P.getTime())?y.created_at:P.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})+" · "+P.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"});return`
        <tr>
          <td class="pos-cell">#${616+y.position}</td>
          <td>
            <div class="name-cell">${M(y.name)}</div>
            <div class="email-cell">${M(y.email)}</div>
          </td>
          <td><span class="role-badge">${M(st[y.role]||y.role)}</span></td>
          <td>${y.company?M(y.company):'<span class="muted">—</span>'}</td>
          <td>${y.github?`<a href="${M(y.github)}" target="_blank" rel="noopener" class="github-link">${M(y.github.replace(/^https?:\/\/(www\.)?github\.com\//,"@"))}</a>`:'<span class="muted">—</span>'}</td>
          ...
          <td>
            ${y.email_sent===1?'<span class="role-badge" style="background:#059669;color:#fff;">Sent</span>':`<div style="display:flex;gap:8px;align-items:center;">
                   ...
                   <form method="post" action="/admin/retrigger" style="margin:0;">
                     <input type="hidden" name="email" value="${M(y.email)}" />
      ...
      <div class="role-bar-row">
        <div class="role-bar-label">${M(st[y.role]||y.role)}</div>
        <div class="role-bar-track"><div class="role-bar-fill" style="width:${He}%"></div></div>
        <div class="role-bar-count">${P}</div>
      </div>`}).join(""),ht=y=>{const P=new URLSearchParams;return s&&P.set("q",s),n&&P.set("role",n),P.set("page",String(E)),Object.entries(y).forEach(([He,Rr])=>P.set(He,String(Rr))),"?"+P.toString()},Er=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
...
<div class="admin-page">
  ...
  <section class="panel">
    ...
    <div class="pagination">
      <span class="page-info">Showing ${I.length?T+1:0}–${T+I.length} of ${v.toLocaleString()}</span>
     ...
     ORDER BY created_at ASC`,i)).rows||[],f=[["Position","Name","Email","Role","Company","GitHub","Joined At (UTC)"].join(",")];for(const v of p)f.push([616+v.position,we(v.name),we(v.email),we(st[v.role]||v.role),we(v.company||""),we(v.github||""),we(v.created_at)].join(","));const w=f.join(`
`);return new Response(w,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="codeward-waitlist-${new Date().toISOString().slice(0,10)}.csv"`}})});re.post("/retrigger",async e=>{const{env:t}=e,s=((await e.req.parseBody()).email||"").toString().trim();if(!s)return e.redirect("/admin");const n=await Ot(e).connect(),i=(await n.query("SELECT name, position FROM waitlist_entries WHERE email = $1",[s])).rows[0];return i&&await hr(t,s,i.name,i.position)?(await n.query("UPDATE waitlist_entries SET email_sent = 1 WHERE email = $1",[s]),e.redirect("/admin?msg=retrigger_success")):e.redirect("/admin?msg=retrigger_failed")});typeof process<"u"&&process.env&&(process.env.POSTGRES_URL=process.env.POSTGRES_URL||process.env.DATABASE_POSTGRES_URL||process.env.DATABASE_URL);function ms(e){const t=new URL(e);return{host:t.hostname,user:decodeURIComponent(t.username),password:decodeURIComponent(t.password),database:t.pathname.replace(/^\//,"")}}function vs(e){const{host:t,user:r,password:s}=ms(e),n=btoa(`${r}:${s}`),a=`https://${t}/sql`;return async function(o,...l){let d="";const p=[];o.forEach((w,v)=>{d+=w,v<l.length&&(p.push(l[v]===void 0?null:l[v]),d+=`$${p.length}`)});const h=await fetch(a,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Basic ${n}`},body:JSON.stringify({query:d,params:p})});if(!h.ok){const w=await h.text();throw new Error(`DB error ${h.status}: ${w}`)}const f=await h.json();return{rows:f.rows??[],rowCount:f.rowCount??0}}}let vt=null;function oe(e){var t,r,s;if(!vt){const n=((t=e==null?void 0:e.env)==null?void 0:t.POSTGRES_URL)||((r=e==null?void 0:e.env)==null?void 0:r.DATABASE_POSTGRES_URL)||((s=e==null?void 0:e.env)==null?void 0:s.DATABASE_URL)||(typeof process<"u"&&process.env?process.env.POSTGRES_URL||process.env.DATABASE_POSTGRES_URL||process.env.DATABASE_URL:void 0);if(!n)throw new Error("No database connection string found in environment");vt=vs(n)}return vt}function bs(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}const me=new Ct;me.onError((e,t)=>(console.error("Hono Global Error:",e),t.json({error:e.message||"Internal Server Error"},500)));me.route("/admin",re);const Re=617;me.get("/api/stats",async e=>{var n;const{env:t}=e,{rows:r}=await oe(e)`SELECT COUNT(*) as cnt FROM waitlist_entries`,s=Number(((n=r[0])==null?void 0:n.cnt)??0);return e.json({count:Re+s})});me.post("/api/join",async e=>{const{env:t}=e;let r;try{r=await e.req.json()}catch{return e.json({error:"Invalid request body"},400)}const s=(r.name||"").toString().trim(),n=(r.email||"").toString().trim().toLowerCase(),a=(r.role||"").toString().trim(),i=(r.company||"").toString().trim(),o=(r.github||"").toString().trim();if(s.length<2)return e.json({error:"Please enter your full name."},400);if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n))return e.json({error:"Please enter a valid email address."},400);if(!a)return e.json({error:"Please select your role."},400);const{rows:l}=await oe(e)`SELECT id, position FROM waitlist_entries WHERE email = ${n}`,d=l[0];if(d){const{rows:v}=await oe(e)`SELECT COUNT(*) as cnt FROM waitlist_entries`,k=v[0];return e.json({alreadyJoined:!0,position:Re+Number(d.position),total:Re+Number((k==null?void 0:k.cnt)??0)})}const{rows:p}=await oe(e)`SELECT COUNT(*) as cnt FROM waitlist_entries`,h=p[0],f=Number((h==null?void 0:h.cnt)??0)+1;await oe(e)`INSERT INTO waitlist_entries (name, email, role, company, github, position) VALUES (${s}, ${n}, ${a}, ${i||null}, ${o||null}, ${f})`;let w=0;return t.RESEND_API_KEY&&await hr(t,n,s,f)&&(w=1),await oe(e)`UPDATE waitlist_entries SET email_sent = ${w} WHERE email = ${n}`,e.json({success:!0,position:Re+f,total:Re+Number((h==null?void 0:h.cnt)??0)+1})});async function hr(e,t,r,s){const n=e.RESEND_API_KEY||process.env.RESEND_API_KEY;if(!n)return!1;try{const a=(Re+s).toLocaleString(),i=`<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8" />
  ...
  <tr>
    <td style="padding:20px 32px 0;text-align:center;">
      ...
      <p style="font-size:14.5px;line-height:23px;color:#c9d1d9;margin:0 0 14px;text-align:left;">
        Hi ${bs(r)} — thanks for requesting early access to Codeward. I wanted to reach out myself rather than send a form reply.
      </p>
      <p style="font-size:14.5px;line-height:23px;color:#c9d1d9;margin:0 0 14px;text-align:left;">
...
</html>`;var le=class extends Error{constructor(e,t){super(e,t),this.name="RequestError"}},ws=e=>e instanceof le?e:new le(e.message,{cause:e}),xs=global.Request,ze=class extends xs{constructor(t,r){var s;typeof t=="object"&&$e in t&&(t=t[$e]()),typeof((s=r==null?void 0:r.body)==null?void 0:s.getReader)<"u"&&(r.duplex??(r.duplex="half")),super(t,r)}},Es=e=>{const t=[],r=e.rawHeaders;for(let s=0;s<r.length;s+=2){const{[s]:n,[s+1]:a}=r;n.charCodeAt(0)!==58&&t.push([n,a])}return new Headers(t)},fr=Symbol("wrapBodyStream"),Rs=(e,t,r,s,n)=>{const a={method:e,headers:r,signal:n.signal};if(e==="TRACE"){a.method="GET";const i=new ze(t,a);return Object.defineProperty(i,"method",{get(){return"TRACE"}}),i}if(!(e==="GET"||e==="HEAD"))if("rawBody"in s&&s.rawBody instanceof Buffer)a.body=new ReadableStream({start(i){i.enqueue(s.rawBody),i.close()}});else if(s[fr]){let i;a.body=new ReadableStream({async pull(o){try{i||(i=It.toWeb(s).getReader());const{done:l,value:d}=await i.read();l?o.close():o.enqueue(d)}catch(l){o.error(l)}}})}else a.body=It.toWeb(s);return new ze(t,a)},$e=Symbol("getRequestCache"),nt=Symbol("requestCache"),at=Symbol("incomingKey"),ot=Symbol("urlKey"),bt=Symbol("headersKey"),J=Symbol("abortControllerKey"),Ss=Symbol("getAbortController"),Xe={get method(){return this[at].method||"GET"},get url(){return this[ot]},get headers(){return this[bt]||(this[bt]=Es(this[at]))},[Ss](){return this[$e](),this[J]},[$e](){return this[J]||(this[J]=new AbortController),this[nt]||(this[nt]=Rs(this.method,this[ot],this.headers,this[at],this[J]))}};["body","bodyUsed","cache","credentials","destination","integrity","mode","redirect","referrer","referrerPolicy","signal","keepalive"].forEach(e=>{Object.defineProperty(Xe,e,{get(){return this[$e]()[e]}})});["arrayBuffer","blob","clone","formData","json","text"].forEach(e=>{Object.defineProperty(Xe,e,{value:function(){return this[$e]()[e]()}})});Object.defineProperty(Xe,Symbol.for("nodejs.util.inspect.custom"),{value:function(e,t,r){const s={method:this.method,url:this.url,headers:this.headers,nativeRequest:this[nt]};return`Request (lightweight) ${r(s,{...t,depth:e==null?null:e-1})}`}});Object.setPrototypeOf(Xe,ze.prototype);var ks=(e,t)=>{const r=Object.create(Xe);r[at]=e;const s=e.url||"";if(s[0]!=="/"&&(s.startsWith("http://")||s.startsWith("https://"))){if(e instanceof Ue)throw new le("Absolute URL for :path is not allowed in HTTP/2");try{const o=new URL(s);r[ot]=o.href}catch(o){throw new le("Invalid absolute URL",{cause:o})}return r}const n=(e instanceof Ue?e.authority:e.headers.host)||t;if(!n)throw new le("Missing host header");let a;if(e instanceof Ue){if(a=e.scheme,!(a==="http"||a==="https"))throw new le("Unsupported scheme")}else a=e.socket&&e.socket.encrypted?"https":"http";const i=new URL(`${a}://${n}${s}`);if(i.hostname.length!==n.length&&i.hostname!==n.replace(/:\d+$/,""))throw new le("Invalid host header");return r[ot]=i.href,r},Me=Symbol("responseCache"),Se=Symbol("getResponseCache"),ce=Symbol("cache"),At=global.Response,Je,W,_e,je=(_e=class{constructor(t,r){m(this,Je);m(this,W);let s;if(u(this,Je,t),r instanceof _e){const n=r[Me];if(n){u(this,W,n),this[Se]();return}else u(this,W,c(r,W)),s=new Headers(c(r,W).headers)}else u(this,W,r);(typeof t=="string"||typeof(t==null?void 0:t.getReader)<"u"||t instanceof Blob||t instanceof Uint8Array)&&(this[ce]=[(r==null?void 0:r.status)||200,t,s||(r==null?void 0:r.headers)])}[Se](){return delete this[ce],this[Me]||(this[Me]=new At(c(this,Je),c(this,W)))}get headers(){const t=this[ce];return t?(t[2]instanceof Headers||(t[2]=new Headers(t[2]||{"content-type":"text/plain; charset=UTF-8"})),t[2]):this[Se]().headers}get status(){var t;return((t=this[ce])==null?void 0:t[0])??this[Se]().status}get ok(){const t=this.status;return t>=200&&t<300}},Je=new WeakMap,W=new WeakMap,_e);["body","bodyUsed","redirected","statusText","trailers","type","url"].forEach(e=>{Object.defineProperty(je.prototype,e,{get(){return this[Se]()[e]}})});["arrayBuffer","blob","clone","formData","json","text"].forEach(e=>{Object.defineProperty(je.prototype,e,{value:function(){return this[Se]()[e]()}})});Object.defineProperty(je.prototype,Symbol.for("nodejs.util.inspect.custom"),{value:function(e,t,r){const s={status:this.status,headers:this.headers,ok:this.ok,nativeResponse:this[Me]};return`Response (lightweight) ${r(s,{...t,depth:e==null?null:e-1})}`}});Object.setPrototypeOf(je,At);Object.setPrototypeOf(je.prototype,At.prototype);async function Cs(e){return Promise.race([e,Promise.resolve().then(()=>Promise.resolve(void 0))])}function ur(e,t,r){const s=o=>{e.cancel(o).catch(()=>{})};return t.on("close",s),t.on("error",s),(r??e.read()).then(i,n),e.closed.finally(()=>{t.off("close",s),t.off("error",s)});function n(o){o&&t.destroy(o)}function a(){e.read().then(i,n)}function i({done:o,value:l}){try{if(o)t.end();else if(!t.write(l))t.once("drain",a);else return e.read().then(i,n)}catch(d){n(d)}}}function Os(e,t){if(e.locked)throw new TypeError("ReadableStream is locked.");return t.destroyed?void 0:ur(e.getReader(),t)}var Et=e=>{const t={};e instanceof Headers||(e=new Headers(e??void 0));const r=[];for(const[s,n]of e)s==="set-cookie"?r.push(n):t[s]=n;return r.length>0&&(t["set-cookie"]=r),t["content-type"]??(t["content-type"]="text/plain; charset=UTF-8"),t},As="x-hono-already-sent";typeof global.crypto>"u"&&(global.crypto=Or);var Tt=Symbol("outgoingEnded"),zt=Symbol("incomingDraining"),Ts=500,Ps=64*1024*1024,yt=e=>{var o,l,d;const t=e;if(e.destroyed||t[zt])return;if(t[zt]=!0,e instanceof Ue){try{(l=(o=e.stream)==null?void 0:o.close)==null||l.call(o,Cr.NGHTTP2_NO_ERROR)}catch{}return}let r=0;const s=()=>{clearTimeout(a),e.off("data",i),e.off("end",s),e.off("error",s)},n=()=>{s();const p=e.socket;p&&!p.destroyed&&p.destroySoon()},a=setTimeout(n,Ts);(d=a.unref)==null||d.call(a);const i=p=>{r+=p.length,r>Ps&&n()};e.on("data",i),e.on("end",s),e.on("error",s),e.resume()},Ls=()=>new Response(null,{status:400}),gr=e=>new Response(null,{status:e instanceof Error&&(e.name==="TimeoutError"||e.constructor.name==="TimeoutError")?504:500}),Rt=(e,t)=>{const r=e instanceof Error?e:new Error("unknown error",{cause:e});r.code==="ERR_STREAM_PREMATURE_CLOSE"?console.info("The user aborted a request."):(console.error(e),t.headersSent||t.writeHead(500,{"Content-Type":"text/plain"}),t.end(`Error: ${r.message}`),t.destroy(r))},mr=e=>{"flushHeaders"in e&&e.writable&&e.flushHeaders()},vr=async(e,t)=>{var i,o;let[r,s,n]=e[ce],a=!1;if(!n)n={"content-type":"text/plain; charset=UTF-8"};else if(n instanceof Headers)a=n.has("content-length"),n=Et(n);else if(Array.isArray(n)){const l=new Headers(n);a=l.has("content-length"),n=Et(l)}else for(const l in n)if(l.length===14&&l.toLowerCase()==="content-length"){a=!0;break}a||(typeof s=="string"?n["Content-Length"]=Buffer.byteLength(s):s instanceof Uint8Array?n["Content-Length"]=s.byteLength:s instanceof Blob&&(n["Content-Length"]=s.size)),t.writeHead(r,n),typeof s=="string"||s instanceof Uint8Array?t.end(s):s instanceof Blob?t.end(new Uint8Array(await s.arrayBuffer())):(mr(t),await((i=Os(s,t))==null?void 0:i.catch(l=>Rt(l,t)))),(o=t[Tt])==null||o.call(t)},_s=e=>typeof e.then=="function",$s=async(e,t,r={})=>{var n;if(_s(e))if(r.errorHandler)try{e=await e}catch(a){const i=await r.errorHandler(a);if(!i)return;e=i}else e=await e.catch(gr);if(ce in e)return vr(e,t);const s=Et(e.headers);if(e.body){const a=e.body.getReader(),i=[];let o=!1,l;if(s["transfer-encoding"]!=="chunked"){let d=2;for(let p=0;p<d;p++){l||(l=a.read());const h=await Cs(l).catch(f=>{console.error(f),o=!0});if(!h){if(p===1){await new Promise(f=>setTimeout(f)),d=3;continue}break}if(l=void 0,h.value&&i.push(h.value),h.done){o=!0;break}}o&&!("content-length"in s)&&(s["content-length"]=i.reduce((p,h)=>p+h.length,0))}t.writeHead(e.status,s),i.forEach(d=>{t.write(d)}),o?t.end():(i.length===0&&mr(t),await ur(a,t,l))}else s[As]||(t.writeHead(e.status,s),t.end());(n=t[Tt])==null||n.call(t)},js=(e,t={})=>{const r=t.autoCleanupIncoming??!0;return t.overrideGlobalObjects!==!1&&global.Request!==ze&&(Object.defineProperty(global,"Request",{value:ze}),Object.defineProperty(global,"Response",{value:je})),async(s,n)=>{let a,i;try{i=ks(s,t.hostname);let o=!r||s.method==="GET"||s.method==="HEAD";if(o||(s[fr]=!0,s.on("end",()=>{o=!0}),s instanceof Ue&&(n[Tt]=()=>{o||setTimeout(()=>{o||setTimeout(()=>{yt(s)})})}),n.on("finish",()=>{o||yt(s)})),n.on("close",()=>{i[J]&&(s.errored?i[J].abort(s.errored.toString()):n.writableFinished||i[J].abort("Client connection prematurely closed.")),o||setTimeout(()=>{o||setTimeout(()=>{yt(s)})})}),a=e(i,{incoming:s,outgoing:n}),ce in a)return vr(a,n)}catch(o){if(a)return Rt(o,n);if(t.errorHandler){if(a=await t.errorHandler(i?o:ws(o)),!a)return}else i?a=gr(o):a=Ls()}try{return await $s(a,n,t)}catch(o){return Rt(o,n)}}},Ds=e=>js(e.fetch);const St=new Ct,Hs=Object.assign({"/src/index.tsx":me});let br=!1;for(const[,e]of Object.entries(Hs))e&&(St.all("*",t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),St.notFound(t=>{let r;try{r=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,r)}),br=!0);if(!br)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");const Fs=Ds(St);export{Fs as default};
